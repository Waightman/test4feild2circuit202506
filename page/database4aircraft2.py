...
def create_connection():
    # 修复隐患：增加 timeout 防止并发死锁，强制开启外键确保级联删除生效
    conn = sqlite3.connect('aircraft_lightning.db', timeout=10)
...
def delete_lightning_zone():
    ...
    # 2. 查询逻辑
    if st.button("查询", key="lz_del_search_btn"):
        conn = create_connection()
    ...
    # 3. 结果显示与批量删除
    if st.session_state['lz_delete_search_result'] is not None:
        df_origin = st.session_state['lz_delete_search_result']
        ...
        if df_origin.empty:
            ...
            if not selected_rows.empty:
                st.warning(
                    f"⚠️ 您已选中 **{len(selected_rows)}** 个飞机型号的分区数据。\n\n**注意：这将同时彻底删除它们在数据库中关联的所有视图图片！** 此操作不可逆，请确认！")

                if st.button("🗑️ 确认彻底删除选中项", type="primary"):
                    try:
                        conn = create_connection()
                        cursor = conn.cursor()
                        ...
                        st.success(f"成功删除 {len(ids_to_delete)} 条主记录及其所有关联的视图数据！")
                        st.session_state['lz_delete_search_result'] = None
                        st.rerun()
                    except Exception as e:
...
def indirect_effects_page(operation):
    ...
    if operation == "查看数据":
        ...
        with tab1:
            add_indirect_effect()
        with tab2:
            batch_add_indirect_effects()
        with tab3:
            hybrid_batch_add_indirect_effects()
        with tab4:
            excel_bulk_file_management()
...
def generate_filename_from_record(record):
    filename_fields = [
        ...
        record.get('test_point', ''),
        record.get('current_in_out', ''),
        record.get('voltage_probe_point', ''),
        record.get('waveform_type', ''),
        record.get('test_object_type', ''),
        record.get('induced_waveform', ''),
        record.get('data_domain', ''),
        record.get('data_type', ''),
        record.get('data_unit', '')
    ]
    ...
    if valid_parts:
        ...
        return f"data_record_{record.get('id', 'unknown')}.dat"
...
def view_indirect_effects():
    ...
    if st.button("查询"):
        conn = create_connection()
        # 修复 OOM 内存溢出风险：刻意排除了 data_file 字段
        query = """
    ...
    if st.session_state['ie_search_result'] is not None:
        ...
        if df_origin.empty:
            ...
            edited_df = st.data_editor(
                ...
                disabled=["id", "aircraft_model", "test_point", "waveform_type"],
                hide_index=True,
                use_container_width=True
            )
                    ...
                    if st.button("生成选中数据的压缩包 (ZIP)"):
                        if selected_rows.empty:
                            ...
                            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                                # 修复 OOM 隐患：按需从数据库中提取文件本身
                                conn_zip = create_connection()
                                cursor_zip = conn_zip.cursor()
                                cursor_zip.execute("SELECT * FROM indirect_effects LIMIT 1")
                                col_names = [description[0] for description in cursor_zip.description]

                                for index, row in selected_rows.iterrows():
                                    record_id = int(row['id'])
                                    cursor_zip.execute("SELECT * FROM indirect_effects WHERE id = ?", (record_id,))
                                    db_record = cursor_zip.fetchone()

                                    if db_record:
                                        original_record = dict(zip(col_names, db_record))
                                        if original_record.get('data_file'):
            ...
            # 修复 OOM 隐患：按需从数据库获取单条数据的全部字段内容
            conn_single = create_connection()
            cursor_single = conn_single.cursor()
            cursor_single.execute("SELECT * FROM indirect_effects WHERE id = ?", (int(selected_id),))
            db_record = cursor_single.fetchone()

            if db_record:
                col_names = [description[0] for description in cursor_single.description]
                selected_record = dict(zip(col_names, db_record))
            else:
                selected_record = None
            conn_single.close()

            if selected_record and selected_record.get('data_file') is not None:
                try:
...
def delete_indirect_effect():
    ...
    # 2. 查询逻辑
    if st.button("查询", key="del_search_btn"):
        conn = create_connection()
        query = """
            SELECT id, aircraft_model, test_point, current_in_out, voltage_probe_point, 
                   waveform_type, induced_waveform, test_object_type, data_domain, 
    ...
    # 3. 结果显示与批量删除
    if st.session_state['ie_delete_search_result'] is not None:
        df_origin = st.session_state['ie_delete_search_result']
        ...
        if df_origin.empty:
            ...
            st.markdown("---")
            if not selected_rows.empty:
                st.warning(f"⚠️ 您已选中 **{len(selected_rows)}** 条数据。删除操作不可逆，请确认！")

                if st.button("🗑️ 确认删除选中项", type="primary"):
                    try:
                        conn = create_connection()
                        cursor = conn.cursor()
...
def hybrid_batch_add_indirect_effects():
    ...
    st.info("""
    💡 **操作提示**：
    在下方设置本次导入的**共同字段**。如果某个字段留空或选择 `<从文件名提取>`，系统将严格按照以下 **10个字段的顺序**，依次从文件名（以下划线 `_` 分隔）中提取内容来填补缺失项。
    
    📋 **标准字段提取顺序**：
    `1.飞机型号` ➔ `2.测试点编号` ➔ `3.电流入/出点` ➔ `4.远端连接器(若开启)` ➔ `5.激励波形` ➔ `6.感应波形` ➔ `7.被测对象` ➔ `8.数据类型` ➔ `9.单位` ➔ `10.数据域`
    
    *(举例：如果您固定了“1.飞机型号”为 AG600，且文件名是 `TP01_LeftWing_Conn2...`，系统会自动将 `TP01` 识别为“2.测试点”，`LeftWing` 识别为“3.电流入/出点”，以此类推。)*
    """)
...
def smart_parse_filename(filename):
    ...
    all_waves = [w.upper() for w in all_waves]
