...
class VectorFitting:
    ...
    def generate_spice_subcircuit_s(self, fitted_model_name: str = "s_equivalent",
        ...
        -------
        str
            The content of the SPICE subcircuit file as a string.
        """
        content = []
        ...
        content.append('* Created using scikit-rf vectorFitting.py\n\n')
        ...
        for n in range(self.network.nports):
            ...
            # CCVS implementing the reflected wave b.
            content.append(f'H_b_{n + 1} a{n + 1} {ref_nodes[n]} V_c_{n + 1} {2.0 * sqrt_Z0_n}\n')

            content.append(f'* Differential incident wave a sources for transfer from port {n + 1}\n')

            # CCVS and VCVS driving the transfer admittances with incident wave a
            content.append(f'H_p_{n + 1} nt_p_{n + 1} nts_p_{n + 1} H_b_{n + 1} {0.5 * sqrt_Z0_n}\n')
            content.append(
                f'E_p_{n + 1} nts_p_{n + 1} {ref_nodes[n]} p{n + 1} {ref_nodes[n]} {1.0 / (2.0 * sqrt_Z0_n)}\n')

            # VCVS driving the transfer admittances with -a
            content.append(f'E_n_{n + 1} {ref_nodes[n]} nt_n_{n + 1} nt_p_{n + 1} {ref_nodes[n]} 1\n')
            ...
            for j in range(self.network.nports):
                ...
                for i_pole in range(len(self.poles)):
                    ...
                    if np.imag(pole) == 0.0:
                        ...
                        l = 1 / np.real(residue)
                        r = -1 * np.real(pole) / np.real(residue)
                        content.append(node + f' rl_admittance res={r} ind={l}\n')
                    else:
                        # Complex pole of a conjugate pair; Add rcl_vccs_admittance
                        r = -1 * np.real(pole) / np.real(residue)
                        c = 2 * np.real(residue) / (np.abs(pole) ** 2)
                        ...
                        b = -2 * (np.real(residue) * np.real(pole) + np.imag(residue) * np.imag(pole))
                        gm = b * l * c
                        content.append(node + f' rcl_vccs_admittance res={r} cap={c} ind={l} gm={gm}\n')

        content.append(f'.ENDS {fitted_model_name}\n\n')

        # Subcircuit for an RLCG equivalent admittance of a complex-conjugate pole-residue pair
        content.append('.SUBCKT rcl_vccs_admittance n_pos n_neg res=1e3 cap=1e-9 ind=100e-12 gm=1e-3\n')
        ...
        # Subcircuit for an RL equivalent admittance of a real pole-residue pair
        content.append('.SUBCKT rl_admittance n_pos n_neg res=1e3 ind=100e-12\n')
        content.append('L1 n_pos 1 {ind}\n')
